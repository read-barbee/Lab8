...
summary(elk2$fr.rc.pred2) #this is both 0s and 1s, which is difficult to interpret. These are JUST the FIXED effects predictions (Marginal Effects). type= "response" means probability distribution, (that's what re.form = NA means (this is the default))
```
...
```{r}
elk2$fr.rc.pred3 <- predict(fr.rc, re.form = ~(1|elkuid) , type = "response") #incorporates random effects variance
summary(elk2$fr.rc.pred3)
hist(elk2$fr.rc.pred3)

#re.form = (ctotrisk|elkuid) full random effects of individuals and predation risks. random intercepts and slopes?

#each time you add a level variance estimates increase. Makes predictions more difficult

#compare spatial predictions from naive model and full random effects model--shows you difference between individual and population
```
...
elk2 <-
  elk2 %>% 
  as_tibble() %>% 
  mutate(w=if_else(used==0, 5000,1), # equivalent to number of available points sampled. Should be on the same order as the number of points you're picking?
...
#shows fixed random intercept for zeroes at very high value (essentially infinite)
#negative intercept for quadratic means they select for it up to a point and then selection decreases
```
...
#these confidence intervals are just from fixed effects variances. need to use Mermod for that
...
#fits the inhomogenous poisson point process correctly using true logistic regression
```
...
forage_for_predict_population <-
  tibble(elkuid = NA, #not making predictions for individual elk
...
```{r plot_fixed_preds}
#fixed effects of the quadratic in this population
pop_pred_unfixed %>% 
...
pred_CI <- function(model, newdata=NULL, alpha=0.05) {
  ...
  V <- vcov(model)$cond[-1,-1]   # getting rid of the random intercept  
...
```{r}
#individual-level quadradic predictions
for_predict_ind %>%
  mutate(pred = exp(totalherb_sc*forage_coef + totalherb2_sc*forage2_coef)) %>% 
  ggplot(., aes(x=totalherb, y=pred, color=elkuid)) +
  geom_line(size=1) +
...
#manual prediction ignores random intercept variance--essential!
```
...
#leaving in infinite variance around 0s makes plot a lot more variable
```
...
#predicted probability of USE (0s only)
```
